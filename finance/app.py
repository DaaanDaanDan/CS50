...
def buy():
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("must provide positive share integer")
        ...
        # Update users
        db.execute ("UPDATE users SET cash = cash - :total_shares WHERE id = :user_id", total_shares=total_shares, user_id=session["user_id"])
        ...
        flash(f"Bought {shares} shares of {symbol} for {usd(total_shares)}")
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(rows) != 0:
            return apology("Username already chosen")
